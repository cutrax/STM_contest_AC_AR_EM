...
static void attr_process_data(uint8_t Attribute_Value_Length, uint8_t *Attribute_Value)
{
  ...
  if (ANCS_Context.cmd_ongoing) {
    ...
    Osal_MemCpy(&ANCS_Context.data_buffer[ANCS_Context.buffer_len], Attribute_Value, Attribute_Value_Length);
  ...
  case ANCS_STATE_DISC_SERVICE:
    {
      ...
      Osal_MemCpy(&app.UUID_128, uuid, 16);
...
uint8_t ANCS_ConsumerSetup(uint16_t conn_handle, uint16_t dataBuffer_len, uint8_t *dataBuffer)
{
  ...
  Osal_MemSet(&ANCS_Context, 0, sizeof(ANCS_Context));
...
uint8_t ANCS_GetNotificationAttr(notificationAttr_type attr)
{
  ...
  index++;
  Osal_MemCpy(&ANCS_Context.data_buffer[index], &attr.UID, 4);
  index += 4;
  if (attr.appID_flag) {
  ...
  if (attr.title_flag) {
    ...
    if (attr.title_max_size > 0) {
      Osal_MemCpy(&ANCS_Context.data_buffer[index], &attr.title_max_size, 2);
      index += 2;
    }
  }
  if (attr.subtitle_flag) {
    ...
    if (attr.subtitle_max_size > 0) {
      Osal_MemCpy(&ANCS_Context.data_buffer[index], &attr.subtitle_max_size, 2);
      index += 2;
    }
  }
  if (attr.message_flag) {
    ...
    if (attr.message_max_size > 0) {
      Osal_MemCpy(&ANCS_Context.data_buffer[index], &attr.message_max_size, 2);
      index += 2;
    }
  }
                        ...
                        uint8_t *appID)
{
  ...
  index++;
  Osal_MemCpy(&ANCS_Context.data_buffer[index], appID, appID_len);
  index += appID_len;
...
uint8_t ANCS_PerformNotificationAction(uint32_t notificationUID, ActionID id)
{
  ...
  index++;
  Osal_MemCpy(&ANCS_Context.data_buffer[index], &notificationUID, 4);
  index += 4;
