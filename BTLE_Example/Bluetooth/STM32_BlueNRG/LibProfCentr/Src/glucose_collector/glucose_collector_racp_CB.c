...
void GL_Collector_RACP_Notifications_Storage(uint16_t attr_handle, uint8_t data_length,uint8_t * value)
{
  ...
  Osal_MemCpy (&(glucose_collector_notifications_for_racp[glucoseCollectorContext.number_of_notifications_for_racp].data_value[0]),
               value,
               data_length);
...
{ 
  ...
  if (attr_handle == glucoseCollectorContext.MeasurementCharValHandle)
  {
    ...
    Osal_MemSet(&GlucoseMeasurement, 0 , sizeof(GlucoseMeasurement)); 
    ...
    Osal_MemCpy(&GlucoseMeasurement.baseTime, value, RACP_BASE_TIME_SIZE);
  ...
  {    
    ...
    Osal_MemSet(&GlucoseMeasurementContext, 0 , sizeof(GlucoseMeasurementContext));        
