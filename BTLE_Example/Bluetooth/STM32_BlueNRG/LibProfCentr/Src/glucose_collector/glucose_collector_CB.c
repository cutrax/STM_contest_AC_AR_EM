...
static uint8_t findCharacHandle(uint16_t uuid_searched, uint8_t numCharac, uint8_t *charac)
{
  ...
  { 
    ...
    {
      Osal_MemCpy(&glucoseCollectorContext.MeasurementCharValHandle, &charac[index+4], 2);
    } 
    
    else if ((uuid_searched == GLUCOSE_SERVICE_UUID) && 
        (uuid_charac == GLUCOSE_MEASUREMENT_CONTEXT_CHAR_UUID)) 
    {
      Osal_MemCpy(&glucoseCollectorContext.MeasurementContextCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == GLUCOSE_SERVICE_UUID) && 
         (uuid_charac == GLUCOSE_FEATURE_CHAR_UUID)) 
    {
      Osal_MemCpy(&glucoseCollectorContext.FeatureCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == GLUCOSE_SERVICE_UUID) && 
        (uuid_charac == GLUCOSE_RACP_CHAR_UUID)) 
    {
      Osal_MemCpy(&glucoseCollectorContext.RACPCharValHandle, &charac[index+4], 2);
    } 
    ...
    {
      Osal_MemCpy(&glucoseCollectorContext.ManufacturerNameCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == DEVICE_INFORMATION_SERVICE_UUID) && 
              (uuid_charac == MODEL_NUMBER_UUID)) 
    {
      Osal_MemCpy(&glucoseCollectorContext.ModelNumberCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == DEVICE_INFORMATION_SERVICE_UUID) && 
             (uuid_charac == SYSTEM_ID_UUID)) 
    {
      Osal_MemCpy(&glucoseCollectorContext.SystemIDCharValHandle, &charac[index+4], 2);
    } 
    else 
    {
