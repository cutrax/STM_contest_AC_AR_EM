                                    ...
                                    uint16_t *end_handle)
{
  ...
  for (i=0; i<glucoseCollectorContext.numpeerServices; i++) {
    if (glucoseCollectorContext.peerServices[j] == UUID_TYPE_16) {
      Osal_MemCpy(&uuid, &glucoseCollectorContext.peerServices[j+5], 2);
      j++;
      Osal_MemCpy(start_handle, &glucoseCollectorContext.peerServices[j], 2);
      j += 2;
      Osal_MemCpy(end_handle, &glucoseCollectorContext.peerServices[j], 2);
      j += 2;
...
tBleStatus GL_Collector_Init(glcInitDevType param)
{
  ...
  Osal_MemCpy(&glucoseCollectorContext, 0, sizeof(glucoseCollectorContext_Type));//TBR
  ...
  Osal_MemCpy(glucoseCollectorContext.public_addr, param.public_addr, 6);
                                  ...
                                  tfilterTypeParameter* racp_filter_parameter_2)
{
  ...
  {
    Osal_MemSet(racp_charval,0x00,RACP_COMMAND_MAX_LEN);
