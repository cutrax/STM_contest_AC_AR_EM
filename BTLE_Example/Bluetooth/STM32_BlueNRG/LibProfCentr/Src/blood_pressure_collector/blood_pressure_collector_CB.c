...
static uint8_t findCharacHandle(uint16_t uuid_searched, uint8_t numCharac, uint8_t *charac)
{
  ...
  { 
    ...
    {
      Osal_MemCpy(&bloodPressureCollectorContext.BPMeasurementCharValHandle, &charac[index+4], 2);
    } 
    
    else if ((uuid_searched == BLOOD_PRESSURE_SERVICE_UUID) && 
        (uuid_charac == INTERMEDIATE_CUFF_PRESSURE_CHARAC_UUID)) 
    {
      Osal_MemCpy(&bloodPressureCollectorContext.ICPCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == BLOOD_PRESSURE_SERVICE_UUID) && 
         (uuid_charac == BLOOD_PRESSURE_FEATURE_CHARAC)) 
    {
      Osal_MemCpy(&bloodPressureCollectorContext.BPFeatureCharValHandle, &charac[index+4], 2);
    } 
    ...
    {
      Osal_MemCpy(&bloodPressureCollectorContext.ManufacturerNameCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == DEVICE_INFORMATION_SERVICE_UUID) && 
              (uuid_charac == MODEL_NUMBER_UUID)) 
    {
      Osal_MemCpy(&bloodPressureCollectorContext.ModelNumberCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == DEVICE_INFORMATION_SERVICE_UUID) && 
             (uuid_charac == SYSTEM_ID_UUID)) 
    {
      Osal_MemCpy(&bloodPressureCollectorContext.SystemIDCharValHandle, &charac[index+4], 2);
    } 
    else 
    {
