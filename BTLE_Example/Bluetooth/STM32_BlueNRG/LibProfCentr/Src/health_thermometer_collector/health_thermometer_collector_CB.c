...
static uint8_t findCharacHandle(uint16_t uuid_searched, uint8_t numCharac, uint8_t *charac)
{
  ...
  { 
    ...
    {
      Osal_MemCpy(&healthThermometerCollectorContext.TempMeasurementCharValHandle, &charac[index+4], 2);
    } 
    
    else if ((uuid_searched == HEALTH_THERMOMETER_SERVICE_UUID) && 
        (uuid_charac == TEMPERATURE_TYPE_CHAR_UUID)) 
    {
      Osal_MemCpy(&healthThermometerCollectorContext.TempTypeCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == HEALTH_THERMOMETER_SERVICE_UUID) && 
         (uuid_charac == INTERMEDIATE_TEMPERATURE_CHAR_UUID)) 
    {
      Osal_MemCpy(&healthThermometerCollectorContext.IntermediateTempCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == HEALTH_THERMOMETER_SERVICE_UUID) && 
        (uuid_charac == MEASUREMENT_INTERVAL_CHAR_UUID)) 
    {
      Osal_MemCpy(&healthThermometerCollectorContext.MeasurementIntervalCharValHandle, &charac[index+4], 2);
    } 
    ...
    {
      Osal_MemCpy(&healthThermometerCollectorContext.ManufacturerNameCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == DEVICE_INFORMATION_SERVICE_UUID) && 
              (uuid_charac == MODEL_NUMBER_UUID)) 
    {
      Osal_MemCpy(&healthThermometerCollectorContext.ModelNumberCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == DEVICE_INFORMATION_SERVICE_UUID) && 
             (uuid_charac == SYSTEM_ID_UUID)) 
    {
      Osal_MemCpy(&healthThermometerCollectorContext.SystemIDCharValHandle, &charac[index+4], 2);
    } 
    else 
    {
