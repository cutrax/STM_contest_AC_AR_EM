...
static uint8_t findCharacHandle(uint16_t uuid_searched, uint8_t numCharac, uint8_t *charac)
{
  ...
  { 
    ...
    {
      Osal_MemCpy(&TimeClientContext.currentTimeCharValHandle, &charac[index+4], 2);
    } 
    
    else if ((uuid_searched == CURRENT_TIME_SERVICE_UUID) && 
        (uuid_charac == LOCAL_TIME_INFORMATION_CHAR_UUID)) 
    {
      Osal_MemCpy(&TimeClientContext.localTimeInfoCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == CURRENT_TIME_SERVICE_UUID) && 
         (uuid_charac == REFERENCE_TIME_INFORMATION_CHAR_UUID)) 
    {
      Osal_MemCpy(&TimeClientContext.refTimeInfoCharValHandle, &charac[index+4], 2);
    } 
    /* Next DST service  characteristics */
    else if ((uuid_searched == NEXT_DST_CHANGE_SERVICE_UUID) && 
        (uuid_charac == TIME_WITH_DST_CHAR_UUID)) 
    {
      Osal_MemCpy(&TimeClientContext.timeWithDSTCharValHandle, &charac[index+4], 2);
    } 
    ...
    {
      Osal_MemCpy(&TimeClientContext.timeUpdateCtlPointCharValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == REFERENCE_UPDATE_TIME_SERVICE_UUID) && 
              (uuid_charac == TIME_UPDATE_STATE_CHAR_UUID)) 
    {
      Osal_MemCpy(&TimeClientContext.timeUpdateStateCharValHandle, &charac[index+4], 2);
    } 
    else 
    {
  ...
  case NOTIFICATION_DATA_RECEIVED:
    ...
    if (*status == BLE_STATUS_SUCCESS)
    {
      if (data->attr_handle == TimeClientContext.currentTimeCharValHandle)
      {
        ...
        Osal_MemCpy(&(TimeClientContext.notified_current_time_value), &(data->data_value[0]),CURRENT_TIME_READ_SIZE); 
