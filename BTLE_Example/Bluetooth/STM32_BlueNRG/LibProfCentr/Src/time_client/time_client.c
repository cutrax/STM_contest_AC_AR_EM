                                    ...
                                    uint16_t *end_handle)
{
  ...
  for (i=0; i<TimeClientContext.numpeerServices; i++) {
    if (TimeClientContext.peerServices[j] == UUID_TYPE_16) {
      Osal_MemCpy(&uuid, &TimeClientContext.peerServices[j+5], 2);
      j++;
      Osal_MemCpy(start_handle, &TimeClientContext.peerServices[j], 2);
      j += 2;
      Osal_MemCpy(end_handle, &TimeClientContext.peerServices[j], 2);
      j += 2;
...
tBleStatus TimeClient_Init(tipcInitDevType param)
{
  ...
  Osal_MemCpy(&TimeClientContext, 0, sizeof(TimeClientContext_Type));
  
  Osal_MemCpy(TimeClientContext.public_addr, param.public_addr, 6);
  Osal_MemCpy(initParam.public_addr, param.public_addr, 6);
...
tBleStatus TimeClient_SecuritySet(tipcSecurityType param)
{
  ...
  if (secParam.oob_enable == OOB_AUTH_DATA_PRESENT)
    Osal_MemCpy(secParam.oob_data, param.oob_data, 16);
...
tBleStatus TimeClient_DeviceConnection(tipcConnDevType param)
{
  ...
  Osal_MemCpy(master_param.peer_addr, param.peer_addr, 6);
...
{ 
  if (TimeClientContext.state == TIME_CLIENT_WAIT_READ_CURRENT_TIME_VALUE)
  {
    ...
    /* store read value */
    Osal_MemCpy(&(TimeClientContext.read_current_time_value), data_value,CURRENT_TIME_READ_SIZE);  
  ...
  else if (TimeClientContext.state == TIME_CLIENT_WAIT_READ_LOCAL_TIME_INFO)
  {
    ...
    /* store read value */
    Osal_MemCpy(&(TimeClientContext.read_local_time_value), data_value,LOCAL_TIME_READ_SIZE); 
    /* Display read value according to the characteristic type */
  ...
  else if (TimeClientContext.state == TIME_CLIENT_WAIT_READ_REFERENCE_TIME_INFO)
  {
    ...
    /* store read value */
    Osal_MemCpy(&(TimeClientContext.read_reference_time_value), data_value,REFERENCE_TIME_READ_SIZE);
    /* Display read value according to the characteristic type */
  ...
  else if (TimeClientContext.state == TIME_CLIENT_WAIT_READ_NEXT_DST_CHANGE_TIME)
  {
    ...
    /* store read value */
    Osal_MemCpy(&(TimeClientContext.read_dst_change_time_value), data_value, NEXT_DST_CHANGE_TIME_READ_SIZE); 
    /* Display read value according to the characteristic type */
  ...
  else if (TimeClientContext.state == TIME_CLIENT_WAIT_READ_SERVER_TIME_UPDATE_STATUS)
  {
    ...
    /* store read value */
    Osal_MemCpy(&(TimeClientContext.read_server_time_update_status_value),data_value, SERVER_TIME_UPDATE_STATUS_READ_SIZE); 
    /* Display read value according to the characteristic type */
