...
static uint8_t findCharacHandle(uint16_t uuid_searched, uint8_t numCharac, uint8_t *charac)
{
  ...
  { 
    ...
    {
      Osal_MemCpy(&alertNotificationClientContext.newAlertCategoryValueHandle, &charac[index+4], 2);
    }     
    else if ((uuid_searched == ALERT_NOTIFICATION_SERVICE_UUID) && 
        (uuid_charac == NEW_ALERT_CHAR_UUID)) 
    {
      Osal_MemCpy(&alertNotificationClientContext.newAlertValueHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == ALERT_NOTIFICATION_SERVICE_UUID) && 
         (uuid_charac == SUPPORTED_UNREAD_ALERT_CATEGORY_CHAR_UUID)) 
    {
      Osal_MemCpy(&alertNotificationClientContext.unreadAlertCategoryValueHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == ALERT_NOTIFICATION_SERVICE_UUID) && 
        (uuid_charac == UNREAD_ALERT_STATUS_CHAR_UUID)) 
    {
      Osal_MemCpy(&alertNotificationClientContext.unreadAlertStatusValueHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == ALERT_NOTIFICATION_SERVICE_UUID) && 
         (uuid_charac == ALERT_NOTIFICATION_CONTROL_POINT_CHAR_UUID)) 
    {
      Osal_MemCpy(&alertNotificationClientContext.ctrlPointCharValueHandle, &charac[index+4], 2);
    }  
    else 
    {
