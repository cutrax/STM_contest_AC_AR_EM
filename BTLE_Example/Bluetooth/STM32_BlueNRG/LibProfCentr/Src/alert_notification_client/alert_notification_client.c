                                    ...
                                    uint16_t *end_handle)
{
  ...
  for (i=0; i<alertNotificationClientContext.numpeerServices; i++) {
    if (alertNotificationClientContext.peerServices[j] == UUID_TYPE_16) {
      Osal_MemCpy(&uuid, &alertNotificationClientContext.peerServices[j+5], 2);
      j++;
      Osal_MemCpy(start_handle, &alertNotificationClientContext.peerServices[j], 2);
      j += 2;
      Osal_MemCpy(end_handle, &alertNotificationClientContext.peerServices[j], 2);
      j += 2;
...
tBleStatus ANC_Init(ancInitDevType param)
{
  ...
  Osal_MemCpy(&alertNotificationClientContext, 0, sizeof(alertNotificationClientContext_Type));
  
  Osal_MemCpy(alertNotificationClientContext.public_addr, param.public_addr, 6);
