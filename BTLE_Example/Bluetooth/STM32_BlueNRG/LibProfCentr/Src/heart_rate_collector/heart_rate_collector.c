                                    ...
                                    uint16_t *end_handle)
{
  ...
  for (i=0; i<heartRateCollectorContext.numpeerServices; i++) {
    if (heartRateCollectorContext.peerServices[j] == UUID_TYPE_16) {
      Osal_MemCpy(&uuid, &heartRateCollectorContext.peerServices[j+5], 2);
      j++;
      Osal_MemCpy(start_handle, &heartRateCollectorContext.peerServices[j], 2);
      j += 2;
      Osal_MemCpy(end_handle, &heartRateCollectorContext.peerServices[j], 2);
      j += 2;
...
tBleStatus HRC_Init(hrcInitDevType param)
{
  ...
  Osal_MemCpy(&heartRateCollectorContext, 0, sizeof(heartRateCollectorContext_Type));//TBR
  ...
  Osal_MemCpy(heartRateCollectorContext.public_addr, param.public_addr, 6);
...
void HRC_Notification_Handler(uint16_t attr_handle, uint8_t data_lenght, uint8_t * value)
{
  ...
  if (attr_handle == heartRateCollectorContext.HRMeasurementCharValHandle)
  {
    ...
    Osal_MemSet(&HRMeasurement, 0 , sizeof(HRMeasurement)); 
    ...
    Osal_MemCpy(&heartRateCollectorContext.HRMeasurementValue, &HRMeasurement, sizeof(HRMeasurement));
  }
}
