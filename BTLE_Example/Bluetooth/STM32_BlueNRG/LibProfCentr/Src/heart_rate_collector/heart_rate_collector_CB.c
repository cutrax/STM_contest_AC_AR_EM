...
static uint8_t findCharacHandle(uint16_t uuid_searched, uint8_t numCharac, uint8_t *charac)
{
  ...
  { 
    ...
    {
      Osal_MemCpy(&heartRateCollectorContext.HRMeasurementCharValHandle, &charac[index+4], 2);
    } 
    
    else if ((uuid_searched == HEART_RATE_SERVICE_UUID) && 
        (uuid_charac == SENSOR_LOCATION_UUID)) 
    {
      Osal_MemCpy(&heartRateCollectorContext.BodySensorLocValHandle, &charac[index+4], 2);
    } 
    else if ((uuid_searched == HEART_RATE_SERVICE_UUID) && 
        (uuid_charac == CONTROL_POINT_UUID)) 
    {
      Osal_MemCpy(&heartRateCollectorContext.HRControlPointValHandle, &charac[index+4], 2);
    } 
    ...
    {
      Osal_MemCpy(&heartRateCollectorContext.ManufacturerNameCharValHandle, &charac[index+4], 2);
    } 
    else 
    {
