                                    ...
                                    uint16_t *end_handle)
{
  ...
  for (i=0; i<findMeLocatorContext.numpeerServices; i++) {
    if (findMeLocatorContext.peerServices[j] == UUID_TYPE_16) {
      Osal_MemCpy(&uuid, &findMeLocatorContext.peerServices[j+5], 2);
      j++;
      Osal_MemCpy(start_handle, &findMeLocatorContext.peerServices[j], 2);
      j += 2;
      Osal_MemCpy(end_handle, &findMeLocatorContext.peerServices[j], 2);
      j += 2;
...
tBleStatus FML_Init(fmlInitDevType param)
{
  ...
  Osal_MemCpy(&findMeLocatorContext, 0, sizeof(FindMeLocatorContext_Type));//TBR
  
  Osal_MemCpy(findMeLocatorContext.public_addr, param.public_addr, 6);
