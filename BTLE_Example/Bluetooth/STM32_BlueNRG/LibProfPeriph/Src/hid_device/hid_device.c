                          ...
                          BLE_CALLBACK_FUNCTION_TYPE hidDeviceCallbackFunc)
{
  ...
  Osal_MemSet( &hidDevice, 0, sizeof(tHidDeviceContext) );
  ...
  for(indx = 0;indx < numOfHIDServices;indx++)
  {
    ...
    hidDevice.hidService[indx].pReportMapCharValue = pReportMapCharBuff;
            Osal_MemCpy(pReportMapCharBuff,hidServiceData->reportDesc,hidServiceData->reportMapValueLen);
    ...
    if(hidServiceData->extReportCharRefNum > 0)
    {
      ...
      {
        Osal_MemCpy(&hidDevice.hidService[indx].nonHidServInfo[indx1],&hidServiceData->nonHidServInfo[indx1],sizeof(tnonHIDService));
      }
    }
    /* boot device type */
    ...
    Osal_MemCpy(&hidDevice.hidService[indx].hidInfoChar,&hidServiceData->hidInfoChar,sizeof(hidServiceData->hidInfoChar));
    Osal_MemCpy(pHidInfoCharBuff,(uint8_t *)&hidDevice.hidService[indx].hidInfoChar,sizeof(hidServiceData->hidInfoChar));
  ...
  Osal_MemSet ( &hidDevice.HIDtoBLEInf, 0, sizeof(tProfileInterfaceContext) );
