		    ...
		    BLE_CALLBACK_FUNCTION_TYPE applcb)
{
  ...
  Osal_MemSet(&alertNotificationServer,0,sizeof(alertNotificationServer));
    
  /* register the sensor with BLE main Profile */
  Osal_MemSet(&alertNotificationServer.ANStoBLEInf,0,sizeof(alertNotificationServer.ANStoBLEInf));
...
tBleStatus ANS_Update_New_Alert_Category(uint8_t len,uint8_t* category)
{
     ...
     (ANSProfile_Read_MainStateMachine() >= ANS_IDLE))
  {
    ...
    if(retval == BLE_STATUS_SUCCESS)
    {
      ...
      Osal_MemCpy(alertNotificationServer.alertCategory,category,len);
...
tBleStatus ANS_Update_Unread_Alert_Category(uint8_t len,uint8_t* category)
{
     ...
     (ANSProfile_Read_MainStateMachine() >= ANS_IDLE))
  {
    ...
    if(retval == BLE_STATUS_SUCCESS)
    {
      ...
      Osal_MemCpy(alertNotificationServer.unreadAlertCategory,category,len);
  ...
  case EVT_DISCONN_COMPLETE:
    {
      ...
      Osal_MemSet(alertNotificationServer.notifyNewAlert,0,10);
      Osal_MemSet(alertNotificationServer.notifyUnreadAlert,0,10);
    }
    break;
  case EVT_VENDOR:
    {
